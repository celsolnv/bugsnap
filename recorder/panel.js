...
export function initRecorderPanel(getTabId) {
  ...
  Recorder.on("onStop", (session) => {
    ...
    setStatusText("Gravacao concluida. Baixe o video e o relatorio da sessao.");
  });
  ...
  btnStart.addEventListener("click", async () => {
    ...
    if (!tabId) {
      setStatusText("Nao foi possivel identificar a aba.", true);
      return;
    }

    setStatusText("Aguardando permissao de gravacao...");
    await Recorder.start(tabId);
  });

  btnStop.addEventListener("click", () => {
    setStatusText("Encerrando gravacao...");
    Recorder.stop();
  });
  ...
  btnDownloadJSON.addEventListener("click", () => {
    ...
    Recorder.downloadSessionJSON(buildSessionPayload(lastSession));
  });
  ...
  function renderTimeline(session) {
    ...
    if (allEvents.length === 0) {
      timelineEl.innerHTML = '<div class="timeline-empty">Nenhum evento capturado durante a gravacao</div>';
      return;
    }
